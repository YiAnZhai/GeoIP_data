...
#		print ids_dup
	...
	def insert_into_base(self):
		update_sql = "UPDATE %s as a inner join (SELECT id, dup_flag from %s where fresh=0) as b \
				...
				on b.dup_flag=c.dup_flag set a.update_date=c.start_date, a.update_times=update_times+1, a.refresh_update=1" % (self.origin_table, self.fresh_dup_remove_table, \
					self.fresh_dup_remove_table)
