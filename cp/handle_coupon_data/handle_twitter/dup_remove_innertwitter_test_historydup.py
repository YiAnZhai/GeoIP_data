...
class Dup_remove():
	def __init__(self):
		...
		self.origin_table = config.dup_innertwitter_test_history_table
		# self.origin_table = config.coupon_twitter_table
		self.inner_table = config.dup_innertwitter_test_history_table
	...
	def handle_dup(self):
		...
		sql_select_codecoupon = """SELECT %s, 0 as fresh from %s  \
						UNION ALL SELECT %s, 1 as fresh from %s where landing_page <> "" and landing_page is not null \
...
#		print ids_dup
	...
	def insert_into_base(self):
		...
		f.write("    left: ")
		f.write(str(re_insert_to_base) + "\n    L_rate: " + str(round(100 * re_insert_to_base / float(sys.argv[1]), 1)) + "%\n")
		f.close()
