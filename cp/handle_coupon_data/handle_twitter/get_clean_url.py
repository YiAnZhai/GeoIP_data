...
con = mdb.connect(host='192.168.8.222', user='root', passwd='moma', db='coupon_datacenter', charset='utf8')
cur = con.cursor()

sql = "select id, landing_page from coupon_twitter_today_copy"
cur.execute(sql)
res = cur.fetchall()

sql_update = 'update coupon_twitter_today_copy set url = %s where id = %s'
md5_list = list()
print len(res), res[:2]
count = 0
key_dict = {}
...
for one_res in res[:]:
#	print one_res, '--------------'
	landing_page = one_res[1]
	if not landing_page:
		count += 1
		continue
	tmp_list = landing_page.split('?', 1)
#	print tmp_list 
	if len(tmp_list) == 1:
