...
class TwitterSearchApiSpider(scrapy.Spider):
	name = "twitter_search_api"
...
#		"MYSQL_PASSWD" : 'moma',
	...
	def start_requests(self):
		self.twitter_con = mdb.connect(host=settings.MYSQL_HOST, user=settings.MYSQL_USER, passwd=settings.MYSQL_PASSWD, db=settings.MYSQL_DBNAME, charset='utf8')
		self.twitter_cur = self.twitter_con.cursor()
		self.twitter_cur.execute("SET NAMES utf8")
		sql = """select brand from coupon_brand_fortwitter_search where popular=1 and `pop&api` is null"""
		a = self.twitter_cur.execute(sql)
		brands = self.twitter_cur.fetchall()
		print
		print
		print "............We still have %d brands to get apiinfo from twitter.............." % (len(brands))
		print
		print

		# brands = ["addidas", "target",  "1800"]
		for brand in brands[:]:
			query_code = "%s code" % (brand)
			para_dic = self.para_dic
			para_dic["oauth_timestamp"] = str(time.time())[:10]
			para_dic["q"] = query_code

			query_code = urllib2.quote(query_code)
			url_code = "%s?count=100&lang=en&q=%s&result_type=recent" % (self.url_search_base, query_code)
			...
			del(para_dic["q"])
			request_code = scrapy.Request(url_code, callback=self.parse_code, headers=self.get_header(para_dic, oauth_signature))
			request_code.meta["brand"] = brand
			request_code.meta["coupon_type"] = "code"
			yield request_code
			print url_code

			query_discount = "%s discount" % (brand)
			para_dic["oauth_timestamp"] = str(time.time())[:10]
			para_dic["q"] = query_discount
			query_discount = urllib2.quote(query_discount)
			url_discount = "%s?count=100&lang=en&q=%s&result_type=recent" % (self.url_search_base, query_discount)
			signature_base_string = self.get_signature_base_string("GET", self.url_search_base, para_dic)
			oauth_signature = self.get_hmac_sha1(signature_base_string)
			del(para_dic["q"])
			request_discount = scrapy.Request(url_discount, callback=self.parse_code, headers=self.get_header(para_dic, oauth_signature))
			request_discount.meta["coupon_type"] = "discount"
