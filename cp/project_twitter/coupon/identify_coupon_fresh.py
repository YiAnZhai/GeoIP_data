...
class Tweet():
    ...
    def get_blacklist_domain(self):
        self.connect_db()
        sql = "select domain,type from %s where type = 'neg_tweet_expand_domain' and domain is not null" % (self.negative_domain_table)
...
#           if "porn" != nagetive_domain[1]:
        nagetive_domain_dict = {nagetive_domain[0].lower():["."+nagetive_domain[0].lower(), "/"+nagetive_domain[0].lower()] for nagetive_domain in nagetive_domains}
    ...
    for item_str in item_str_list:
        ...
        if 2 == len(item_model):
            for word in item_model[1].split(","):
                ...
                result_set.add( tuple(sorted(item_model[0].strip().split(",") + list_others)) )
    return result_set
...
def is_coupon(tweet, list_t, search_word_dict):
    text = get_item_value(tweet, 'extended_tweet', "full_text")
    text = text if text else tweet["text"]
...
#    print sure_list
    ...
    extend_url = "****".join([ a["expanded_url"] for a in get_item_value(tweet, "extended_tweet", "entities", "urls") ])
    # print extend_url

    for l in sure_list:
        for l_word in l:
            if " " in l_word or "%" in l_word or "$" in l_word:
                if l_word not in text_lower:
                    break
            elif "off" == l_word or "free" == l_word:
                if l_word not in list_t:
                    break
            elif "" != l_word:
                try:
                    if l_word not in list_t and "%ss" % l_word not in list_t:
                        break
#                   print l_word
#                   print list_t
                except Exception,e:
                    print "wwwwwwwwwwwwwwwwwwwwwww", e
        else:
            for black_domain in domain_black_list:
                for black_domain_pattern in domain_black_list[black_domain]:
                    if extend_url and black_domain_pattern in extend_url or not extend_url and black_domain_pattern in url:
                        return (4, black_domain)

            for common_catword in common_cats:
                if common_catword in text_lower:
                    return 2
#           coupon_model_all[l] += [text]   #for review coupon
            return 0 #super Ture
    ##############################################################################################################################
    ############### go on rule #####################
#     for l in goon_rule_list:
#         for l_word in l:
#             if " " in l_word or "%" in l_word or "$" in l_word:
#                 if l_word not in text_lower:
#                     break
...
#                     print "wwwwwwwwwwwwwwwwwwwwwww", e
#         else:
#             return 100 #go on rule
#     for r in sure_coupon_reg:
#         if re.search( r, text_lower, re.I):
#             return 100
    ##############################################################################################################################
...
def get_time_period(time_ago, times_perday):
    day_periods = [(dt.strptime(time_ago[:8], "%Y%m%d") + datetime.timedelta(days=float(i)/times_perday)).strftime("%Y%m%d-%H%M") \
        for i in range(0, times_perday+1)]
    for index in range(len(day_periods)):
        if time_ago < day_periods[index]:
            return (day_periods[index-1], day_periods[index])


def get_files_in_time_period(files, time_ago, times_perday):
    period_tuple = get_time_period(time_ago, times_perday)
    print period_tuple
    result_files = []
    for f in files:
        time_f = f[-17:-4]
        if time_f >= period_tuple[0] and time_f < period_tuple[1]:
            result_files.append( f )
    return result_files[:]


def get_item_value( item_dict, item, item2="", item3=""):
...
goon_rule_list = set()
for goon_rule in goon_rule_dict.values():
    goon_rule_list.union(get_sure_list(goon_rule))
...
print "likely rules:", search_word_dict
if "__main__" == __name__:
    ...
    # for i in range(1, 2):
        ...
        time_ago = (dt.now() - datetime.timedelta(days=float(i)/times_update_perday)).strftime("%Y%m%d-%H%M")
    #    yestoday = (dt.now() - datetime.timedelta(days=i)).strftime("%Y%m%d")
        t1_t2_period = get_time_period(time_ago, times_update_perday)
        yestoday = "%s_%s" % (t1_t2_period[0], t1_t2_period[1])

        f_coupon = open( os.path.join(coupon_store_path, "%s_coupon.txt_freshdata" % yestoday), "w" )
        f_likecoupon = open(os.path.join(coupon_store_path, "%s_coupon_likely.txt_freshdata" % yestoday), "w")
        f_coupon_bigcat = open(os.path.join(coupon_store_path, "%s_coupon_bigcat.txt_freshdata" % yestoday), "w")
        f_coupon_blacklist = open(os.path.join(coupon_store_path, "%s_coupon_blacklist.txt_freshdata" % yestoday), "w")
        f_longcoupon = open(os.path.join(coupon_store_path, "%s_coupon_longer140.txt_freshdata" % yestoday), "w")
        f_goon_coupon = open(os.path.join(coupon_store_path, "%s_coupon_goon_rule.txt_freshdata" % yestoday), "w")
        
        f_coupon_retweet = open(os.path.join(coupon_store_path, "%s_retweet_coupon.txt_freshdata" % yestoday), "w")
        f_likecoupon_retweet = open(os.path.join(coupon_store_path, "%s_retweet_coupon_likely.txt_freshdata" % yestoday), "w")
        f_coupon_bigcat_retweet = open(os.path.join(coupon_store_path, "%s_retweet_coupon_bigcat.txt_freshdata" % yestoday), "w")
        f_coupon_blacklist_retweet = open(os.path.join(coupon_store_path, "%s_retweet_coupon_blacklist.txt_freshdata" % yestoday), "w")
        f_longcoupon_retweet = open(os.path.join(coupon_store_path, "%s_retweet_coupon_longer140.txt_freshdata" % yestoday), "w")
        f_goon_coupon_retweet = open(os.path.join(coupon_store_path, "%s_retweet_coupon_goon_rule.txt_freshdata" % yestoday), "w")
    ...
    #    destination_dir =[file_name for file_name in dir_list[:] if file_name.startswith("coupon-%s" % yestoday)]
        destination_dir = get_files_in_time_period(dir_list, time_ago, times_update_perday)
            ...
            with open(fname, 'r') as f:
                for line in f:
                    try:
                        ...
                        # if "en" == tweet["lang"]: 
                        # if True:
                            tweet_text = get_item_value(tweet, 'extended_tweet', "full_text")
                            tweet_text = tweet_text if tweet_text else tweet["text"]
                            n_docs += 1
                            # Create a list with all the terms
                            terms_all = [term for term in preprocess( tweet_text ) if term not in stop]
                            # Count hashtags only
                            terms_hash = [term for term in terms_all
                                          if term.startswith('#')]
        
                            terms_for_coupon = [term.strip("#@").lower() for term in terms_all] + [t.lower() for t in terms_hash]
                            # Count user only
                            terms_user = [term for term in terms_all
                                          if term.startswith('@')]
                            ...
                            coupon_flag = is_coupon(tweet, terms_for_coupon, search_word_dict)
                            if 3 == coupon_flag:
                                pass
                            elif 0 == coupon_flag and len( tweet_text ) < 151:
                                f_coupon.write(line)
                                f_coupon.write("\n")
                            elif 1 == coupon_flag:
                                f_likecoupon.write(line)
                                f_likecoupon.write("\n")
                            elif 100 == coupon_flag:
                                f_goon_coupon.write(line)
                                f_goon_coupon.write("\n")
                            elif 2 == coupon_flag and len( tweet_text ) < 151:
                            ...
                            #### long text to review
                            if coupon_flag in [0,2] and len( tweet_text ) > 150:
                                f_longcoupon.write(line)
                                f_longcoupon.write("\n")
        ...
        #                           com[w1][w2] += 1
                            ...
                            terms_all = [term for term in preprocess( tweet_text ) if term not in stop]
                            # Count hashtags only
                            terms_hash = [term for term in preprocess( tweet_text )
                                          if term.startswith('#')]
        
                            terms_for_coupon = [term.strip("#@").lower() for term in terms_all] + [t.lower() for t in terms_hash]
                            # Count user only
                            terms_user = [term for term in preprocess( tweet_text )
                                          if term.startswith('@')]
                            # Count terms only (no hashtags, no mentions)
                            terms_only = [term for term in preprocess( tweet_text )
                            ...
                            if 3 == coupon_flag:
                                ...
                                retweet_goon[tweet["id_str"]] = line
                            elif 2 == coupon_flag and len( tweet_text ) < 151:
                            ...
                            ###### long text
                            if coupon_flag in [0,2] and len( tweet_text ) > 150:
                                retweet_long140[tweet['id_str']] = line
        ...
        for item_goon in retweet_goon:
            f_goon_coupon_retweet.write(retweet_goon[item_goon])
            f_goon_coupon_retweet.write("\n")
        ...
        for item_long140 in retweet_long140:
            f_longcoupon_retweet.write(retweet_long140[item_long140])
            f_longcoupon_retweet.write("\n")
        f_coupon_retweet.close()
        f_likecoupon_retweet.close()
        f_coupon_bigcat_retweet.close()
        f_coupon_blacklist_retweet.close()
        f_longcoupon_retweet.close()
