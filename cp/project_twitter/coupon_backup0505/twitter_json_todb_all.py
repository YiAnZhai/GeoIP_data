...
class Tweet():
	...
	def insert_user(self):
		...
		i = sys.argv[1]
		file_mintime = "20160610"
		for parent in parents:
			if parent.startswith("2016"):
				file_mintime = parent[:8]
				break
		day = (dt.strptime(file_mintime, "%Y%m%d") + datetime.timedelta(days=int(i))).strftime("%Y%m%d")

		parent_dir_files = [aa for aa in parents if aa.startswith(day) and 0 == aa.count("likely")]
		print parent_dir_files
